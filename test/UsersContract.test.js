...
describe('The UsersContract', () => {
  ...
  it('should join a user', async () => {
    let name = "Jose";
    let suername = "Landeras";

    await usersContract.methods.join(name, suername).send({from: accounts[0], gas: '5000000'});
  });
  ...
  it('should retrive a user', async () => {
    ...
    assert.equal(suername, user[1]);

  });

  it('should not allow joining an account twice', async () => {
    
    await usersContract.methods.join("Jose", "Peraza").send({from: accounts[1], gas: '5000000'});
    
    try {
      await usersContract.methods.join("Gaby", "Hernandez").send({from: accounts[1], gas: '5000000'});
